...
class JAXAgent(embodied.Agent):
  ...
  def __init__(self, agent_cls, obs_space, act_space, step, config):
    ...
    # TODO: (Ed) assumes we have a single GPU that runs the JAX and enviroment code.
    if self.single_device and self.config.dynamic_prealloc and self.config.platform == 'gpu' and self.envs_config.gpu_mb_usage > -1:
      def determine_fraction(env_memory):
          import nvidia_smi
          nvidia_smi.nvmlInit()
          handle = nvidia_smi.nvmlDeviceGetHandleByIndex(0)  # 0 is GPU id
          info = nvidia_smi.nvmlDeviceGetMemoryInfo(handle)

          total_gpu_memory = info.total / (1024 * 1024)  # Convert to MB
          preallocate_memory = total_gpu_memory - env_memory  # Subtract gym environment memory
          # convert to string decimal format, like '0.8'
          ratio = str(preallocate_memory / total_gpu_memory)
          return total_gpu_memory, preallocate_memory,ratio
      
      # account for train, eval, full_eval envs
      slack = 100 # give extra mb just in case.
      total_env_memory = (self.envs_config.gpu_mb_usage * self.envs_config.amount * 3) + slack
      total_gpu_memory, preallocate_memory, ratio = determine_fraction(total_env_memory)
      os.environ['XLA_PYTHON_CLIENT_MEM_FRACTION'] = ratio
      print('GPU total memory:', total_gpu_memory, 'mb')
      print('Projected Env GPU memory usage:', total_env_memory, 'mb')
      print('Available GPU memory for JAX:', preallocate_memory, 'mb')
      print('JAX GPU fraction:', os.environ['XLA_PYTHON_CLIENT_MEM_FRACTION'])
