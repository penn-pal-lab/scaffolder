...
class DualRSSM(nj.Module):
  ...
  def initial(self, bs):
    if self._classes:
      state = dict(
          ...
          logit=jnp.zeros([bs, self._stoch, self._classes], f32),
          stoch=jnp.zeros([bs, self._stoch, self._classes], f32),
          full_deter=jnp.zeros([bs, self._deter], f32),
          full_logit=jnp.zeros([bs, self._stoch, self._classes], f32),
          full_stoch=jnp.zeros([bs, self._stoch, self._classes], f32))
    else:
      state = dict(
          deter=jnp.zeros([bs, self._deter], f32),
          mean=jnp.zeros([bs, self._stoch], f32),
          ...
          stoch=jnp.zeros([bs, self._stoch], f32),
          full_deter=jnp.zeros([bs, self._deter], f32),
          full_mean=jnp.zeros([bs, self._stoch], f32),
          full_std=jnp.ones([bs, self._stoch], f32),
          full_stoch=jnp.zeros([bs, self._stoch], f32))
    if self._initial == 'zeros':
      return cast(state)
    elif self._initial == 'learned':
      deter = self.get('initial', jnp.zeros, state['deter'][0].shape, f32)
      state['deter'] = jnp.repeat(jnp.tanh(deter)[None], bs, 0)
      state['stoch'] = self.get_stoch(cast(state['deter']))
      full_deter = self.get('full_initial', jnp.zeros, state['full_deter'][0].shape, f32)
      state['full_deter'] = jnp.repeat(jnp.tanh(full_deter)[None], bs, 0)
      state['full_stoch'] = self.get_full_stoch(cast(state['full_deter']))
      return cast(state)
    else:
      raise NotImplementedError(self._initial)

  def observe_combined(self, embed, full_embed, action, is_first, combined_state=None):
    """Observes the full observation o^+=[e, e^p] and returns the state s and full state s^+. combined_state is the union dict of s dict and s^+ dict."""
    swap = lambda x: x.transpose([1, 0] + list(range(2, len(x.shape))))
    if combined_state is None:
      combined_state = self.initial(action.shape[0])
    state = {k:v for k,v in combined_state.items() if not 'full_' in k}
    full_state = {k:v for k,v in combined_state.items() if 'full_' in k}

    def step(prev, inputs):
      # prev is the output of self.obs_both_step from the previous computation.
      post = prev[0]
      full_post = prev[2]
      combined_post = {**{k:v for k,v in post.items() if not 'full_' in k}, **{k:v for k,v in full_post.items() if 'full_' in k}}
      return self.obs_combined_step(combined_post, *inputs)

    inputs = swap(action), swap(embed), swap(full_embed), swap(is_first)
    start = state, state, full_state, full_state # initial post, prior, full_post, full_prior
    post, prior, full_post, full_prior = jaxutils.scan(step, inputs, start, self._unroll)
    post = {k: swap(v) for k, v in post.items()}
    prior = {k: swap(v) for k, v in prior.items()}

    full_post = {k: swap(v) for k, v in full_post.items()}
    full_prior = {k: swap(v) for k, v in full_prior.items()}
    return post, prior, full_post, full_prior

  def obs_combined_step(self, prev_combined_state, prev_action, embed, full_embed, is_first):
    is_first = cast(is_first)
    prev_action = cast(prev_action)
    if self._action_clip > 0.0:
      prev_action *= sg(self._action_clip / jnp.maximum(
          self._action_clip, jnp.abs(prev_action)))
    # set the prev_action to 0 if is_first.
    prev_combined_state, prev_action = jax.tree_util.tree_map(
        lambda x: self._mask(x, 1.0 - is_first), (prev_combined_state, prev_action))
    # set the prev state to initial state values if is_first.
    prev_combined_state = jax.tree_util.tree_map(
        lambda x, y: x + self._mask(y, is_first),
        prev_combined_state, self.initial(len(is_first)))
    prior, full_prior = self.img_combined_step(prev_combined_state, prev_action)
    x = jnp.concatenate([prior['deter'], embed], -1)
    x = self.get('obs_out', Linear, **self._kw)(x)
    stats = self._stats('obs_stats', x)
    dist = self.get_dist(stats)
    ...
    # p(s^+ | s^+_t-1, a_t-1, e^+)
    full_x = jnp.concatenate([full_prior['full_deter'], full_embed], -1)
    full_x = self.get('full_obs_out', Linear, **self._kw)(full_x)
    full_stats = self._stats('full_obs_stats', full_x)
    full_dist = self.get_dist(full_stats)
    full_stoch = full_dist.sample(seed=nj.rng())
    full_stats = {f"full_{k}":v for k,v in full_stats.items()}
    full_post = {'full_stoch': full_stoch, 'full_deter': full_prior['full_deter'], **full_stats}
    return cast(post), cast(prior), cast(full_post), cast(full_prior)

  def obs_step(self, prev_state, prev_action, embed, is_first):
    # used by full imagination of policy
    is_first = cast(is_first)
    prev_action = cast(prev_action)
    if self._action_clip > 0.0:
      prev_action *= sg(self._action_clip / jnp.maximum(
    ...
    prev_state, prev_action = jax.tree_util.tree_map(
        lambda x: self._mask(x, 1.0 - is_first), (prev_state, prev_action))
    def set_initial(prev_state, initial):
      # If is_first, then prev_state is already set to 0s from above, so add the initial values.
      # If not, then add 0s, which does nothing.
      return prev_state + self._mask(initial, is_first) 
    initial = {k:v for k,v in self.initial(len(is_first)).items() if not 'full_' in k}
    prev_state = jax.tree_util.tree_map(
        set_initial, prev_state, initial)
    prior = self.img_step(prev_state, prev_action)
    x = jnp.concatenate([prior['deter'], embed], -1)
    x = self.get('obs_out', Linear, **self._kw)(x)
    stats = self._stats('obs_stats', x)
  ...
  def img_full_step(self, prev_full_state, prev_action):
    # used by imagination of full policy
    prev_full_stoch = prev_full_state['full_stoch']
    prev_action = cast(prev_action)
    if self._action_clip > 0.0:
      prev_action *= sg(self._action_clip / jnp.maximum(
          self._action_clip, jnp.abs(prev_action)))
    if self._classes:
      shape = prev_full_stoch.shape[:-2] + (self._stoch * self._classes,)
      prev_full_stoch = prev_full_stoch.reshape(shape)
    if len(prev_action.shape) > len(prev_full_stoch.shape):  # 2D actions.
      shape = prev_action.shape[:-2] + (np.prod(prev_action.shape[-2:]),)
      prev_action = prev_action.reshape(shape)
    x = jnp.concatenate([prev_full_stoch, prev_action], -1)
    x = self.get('full_img_in', Linear, **self._kw)(x)
    x, deter = self._full_gru(x, prev_full_state['full_deter'])
    x = self.get('full_img_out', Linear, **self._kw)(x)
    full_stats = self._stats('full_img_stats', x)
    dist = self.get_dist(full_stats)
    stoch = dist.sample(seed=nj.rng())
    full_stats = {f"full_{k}":v for k,v in full_stats.items()}
    prior = {'full_stoch': stoch, 'full_deter': deter, **full_stats}
    return cast(prior)

  def img_combined_step(self, prev_combined_state, prev_action):
    # used by observation function
    prev_stoch = prev_combined_state['stoch']
    prev_full_stoch = prev_combined_state['full_stoch']
    prev_action = cast(prev_action)
    if self._action_clip > 0.0:
      prev_action *= sg(self._action_clip / jnp.maximum(
          self._action_clip, jnp.abs(prev_action)))
    ...
    x = jnp.concatenate([prev_stoch, prev_action], -1)
    x = self.get('img_in', Linear, **self._kw)(x)
    x, deter = self._gru(x, prev_combined_state['deter'])
    x = self.get('img_out', Linear, **self._kw)(x)
    stats = self._stats('img_stats', x)
    dist = self.get_dist(stats)
    stoch = dist.sample(seed=nj.rng())
    ...
    full_x = jnp.concatenate([prev_full_stoch, prev_action], -1)
    full_x = self.get('full_img_in', Linear, **self._kw)(full_x)
    full_x, full_deter = self._full_gru(full_x, prev_combined_state['full_deter'])
    full_x = self.get('full_img_out', Linear, **self._kw)(full_x)
    full_stats = self._stats('full_img_stats', full_x)
    full_dist = self.get_dist(full_stats)
    full_stoch = full_dist.sample(seed=nj.rng())
    # add full_ to all keys in full_stats
    full_stats = {f"full_{k}":v for k, v in full_stats.items()}
    full_prior = {'full_stoch': full_stoch, 'full_deter': full_deter, **full_stats}
    return cast(prior), cast(full_prior)

  def imagine_combined(self, action, combined_state=None):
    """Rollout the dynamics model using the given action sequence. Used by report function."""
    swap = lambda x: x.transpose([1, 0] + list(range(2, len(x.shape))))
    combined_state = self.initial(action.shape[0]) if combined_state is None else combined_state
    state = {k:v for k,v in combined_state.items() if not k.startswith('full_')}
    full_state = {k:v for k,v in combined_state.items() if k.startswith('full_')}
    assert isinstance(state, dict), state
    action = swap(action)
    def step(prev, inputs):
      prior, full_prior = prev
      combined_state = {**prior, **full_prior}
      return self.img_combined_step(combined_state, inputs)
    start = state, full_state
    prior, full_prior = jaxutils.scan(step, action, start, self._unroll)
    prior = {k: swap(v) for k, v in prior.items()}
    full_prior = {k: swap(v) for k, v in full_prior.items()}
    return prior, full_prior
  ...
  def get_full_stoch(self, deter):
    x = self.get('full_img_out', Linear, **self._kw)(deter)
    stats = self._stats('full_img_stats', x)
    dist = self.get_dist(stats)
    return cast(dist.mode())
  ...
  def _full_gru(self, x, deter):
    x = jnp.concatenate([deter, x], -1)
    kw = {**self._kw, 'act': 'none', 'units': 3 * self._deter}
    x = self.get('full_gru', Linear, **kw)(x)
    reset, cand, update = jnp.split(x, 3, -1)
    reset = jax.nn.sigmoid(reset)
    cand = jnp.tanh(reset * cand)
    update = jax.nn.sigmoid(update - 1)
...
class DualMultiEncoder(nj.Module):
  def __init__(
      self, shapes, full_cnn_keys=r'.*', cnn_keys=r'.*', full_mlp_keys=r'*.', mlp_keys=r'.*', mlp_layers=4,
      mlp_units=512, cnn='resize', cnn_depth=48,
      cnn_blocks=2, resize='stride',
      symlog_inputs=False, minres=4, **kw):
    excluded = ('is_first', 'is_last')
    ...
    self.cnn_shapes = {k: v for k, v in shapes.items() if (
        len(v) == 3 and re.match(cnn_keys, k))}
    self.full_cnn_shapes = {k: v for k, v in shapes.items() if (
        len(v) == 3 and re.match(full_cnn_keys, k))}
    self.mlp_shapes = {k: v for k, v in shapes.items() if (
        len(v) in (1, 2) and re.match(mlp_keys, k))}
    self.full_mlp_shapes = {k: v for k, v in shapes.items() if (
        len(v) in (1, 2) and re.match(full_mlp_keys, k))}
    self.shapes = {**self.cnn_shapes, **self.mlp_shapes}
    self.full_shapes = {**self.full_cnn_shapes, **self.full_mlp_shapes}
    print('CNN Encoder shapes:', self.cnn_shapes)
    print('MLP Encoder shapes:', self.mlp_shapes)
    cnn_embed_size = 0
    if len(self.cnn_shapes) > 0:
      img_res = list(self.cnn_shapes.values())[0][0]  # assumes all imgs are square and have same resolution
      layers = np.log2(img_res) - np.log2(minres)
      cnn_embed_size = int(minres**2 * cnn_depth * np.power(2, layers - 1))
    mlp_embed_size = mlp_units * (len(self.mlp_shapes) > 0)
    self.embed_size = cnn_embed_size + mlp_embed_size
    print('MultiEncoder embed size:', self.embed_size)
    print('Full CNN Encoder shapes:', self.full_cnn_shapes)
    print('Full MLP Encoder shapes:', self.full_mlp_shapes)

    cnn_kw = {**kw, 'minres': minres, 'name': 'cnn'}
    full_cnn_kw = {**kw, 'minres': minres, 'name': 'full_cnn'}
    mlp_kw = {**kw, 'symlog_inputs': symlog_inputs, 'name': 'mlp'}
    full_mlp_kw = {**kw, 'symlog_inputs': symlog_inputs, 'name': 'full_mlp'}
    if cnn == 'resnet':
      self._cnn = ImageEncoderResnet(cnn_depth, cnn_blocks, resize, **cnn_kw)
      self._full_cnn = ImageEncoderResnet(cnn_depth, cnn_blocks, resize, **full_cnn_kw)
    else:
      raise NotImplementedError(cnn)
    if self.mlp_shapes:
      self._mlp = MLP(None, mlp_layers, mlp_units, dist='none', **mlp_kw)
    if self.full_mlp_shapes:
      self.full_mlp = MLP(None, mlp_layers, mlp_units, dist='none', **full_mlp_kw)
  ...
  def __call__(self, data):
    ...
    if self.full_cnn_shapes:
      inputs = jnp.concatenate([data[k] for k in self.full_cnn_shapes], -1)
      output = self._full_cnn(inputs)
      output = output.reshape((output.shape[0], -1))
    ...
    if self.full_mlp_shapes:
      inputs = [
          data[k][..., None] if len(self.full_shapes[k]) == 0 else data[k]
          for k in self.full_mlp_shapes]
      inputs = jnp.concatenate([x.astype(f32) for x in inputs], -1)
      inputs = jaxutils.cast_to_compute(inputs)
      full_outputs.append(self.full_mlp(inputs))

    outputs = jnp.concatenate(outputs, -1)
    outputs = outputs.reshape(batch_dims + outputs.shape[1:])

    full_outputs = jnp.concatenate(full_outputs, -1)
    full_outputs = full_outputs.reshape(batch_dims + full_outputs.shape[1:])
