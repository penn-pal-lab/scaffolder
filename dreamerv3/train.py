...
def main(argv=None):
  ...
  try:
    ...
    elif args.script == 'train_full_eval':
      replay = make_replay(config, logdir / 'replay')
      eval_replay = make_replay(config, logdir / 'eval_replay', is_eval=True)
      full_eval_replay = make_replay(config, logdir / 'full_eval_replay', is_eval=True)
      env = make_envs(config)
      eval_env = make_envs(config)  # mode='eval'
      full_eval_env = make_envs(config)  # mode='eval'
      cleanup += [env, eval_env, full_eval_env]
      agent = agt.Agent(env.obs_space, env.act_space, step, config)
      embodied.run.train_full_eval(
          agent, env, eval_env, full_eval_env, replay, eval_replay, full_eval_replay, logger, args)
...
def make_env(config, **overrides):
  ...
  suite, task = config.task.split('_', 1)
  if 'gibbon' in env.lower():
    try:
      import brachiation
    except Exception as e:
      print('Failed to import brachiation, exiting...')
      exit()
